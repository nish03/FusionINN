...
for i in range(6):
        fused_img = cv2.imread('/home/h1/s8993054/INN_Fusion/DDFM/output/clinical_output_full_slice/' + str(i) + '.png').astype('float32')
...
with torch.no_grad():
    for idx in range(clinical_fused_tensor.shape[0]):
        ...
        if count % 100 == 0:
            print(count)   
    torch.save(recon_val, 'clinical_recon_tensor_DDFM_on_INN_full_slice.pt')
...
for i in range(clinical_fused_tensor.shape[0]):
    plt.imsave('/home/h1/s8993054/INN_Fusion/Clinical_Fusion/Recon_DDFM_Flair_full_slice/Recon_Flair_' + str(i) + '.png', clinical_recon_numpy[i,0,], cmap = 'gray', vmin = 0.0, vmax = 1.0)
    plt.clf()
    plt.imsave('/home/h1/s8993054/INN_Fusion/Clinical_Fusion/Recon_DDFM_DWI_full_slice/Recon_DWI_' + str(i) + '.png', clinical_recon_numpy[i,1,], cmap = 'gray', vmin = 0.0, vmax =1.0)
    plt.close()
