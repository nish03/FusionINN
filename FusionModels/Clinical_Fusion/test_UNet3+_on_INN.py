...
with torch.no_grad():
    for idx in range(clinical_fused_tensor.shape[0]):
        ...
        if count % 100 == 0:
            print(count)   
    torch.save(recon_val, 'clinical_recon_tensor_UNet3+_on_INN.pt')
    
clinical_recon_numpy = recon_val.cpu().squeeze()

    
#save the clinical images
for i in range(clinical_fused_numpy.shape[0]):
    plt.imsave('/home/h1/s8993054/INN_Fusion/Clinical_Fusion/Recon_UNet3+_Flair/Recon_Flair_' + str(i) + '.png', clinical_recon_numpy[i,0,], cmap = 'gray')
    plt.clf()
    plt.imsave('/home/h1/s8993054/INN_Fusion/Clinical_Fusion/Recon_UNet3+_T1ce/Recon_T1ce_' + str(i) + '.png', clinical_recon_numpy[i,1,], cmap = 'gray')
    plt.close()
